...
def make_matte(image,ckpt_path):
    ...
    matte_image = Image.fromarray(((matte * 255).astype('uint8')),
                        mode='L')
...
def display(image,matte):
    ...
    foreground_image = Image.fromarray(np.uint8(foreground)).resize((w, h))
...
matte = make_matte(image,ckpt_path)
display(image,matte)
...
#         Image.fromarray(((matte * 255).astype('uint8')),
#                         mode='L').save(os.path.join(args.output_path, matte_name))
